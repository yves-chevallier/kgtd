/**
 * Initializes graphics (compiles display lists)
 *
...
static float noob_points[] = {-NOOB_SIZE / 2, -NOOB_SIZE / 2, -NOOB_SIZE / 2,
                              NOOB_SIZE / 2,  NOOB_SIZE / 2,  NOOB_SIZE / 2,
                              NOOB_SIZE / 2,  -NOOB_SIZE / 2};

// Vertex array for the X
static float x_points[] = {0.0,
                           0.0,
                           X_WIDTH,
                           0.0,
                           X_SIZE + X_WIDTH,
...
// Vertex array for control bar occluder
static float ctl_points[] = {
    0,
    LEVEL_BAR - BOT_BAR_BUFFER + 3,
    0,
    YRES,
    XRES,
    YRES,
...
// Vertex array for the wave indicator box
static float wave_points[] = {0.0,
                              0.0,
                              0.0,
                              LEVEL_BAR_HEIGHT,
                              LEVEL_BAR_WIDTH - 10,
...
// Icons
static float icon_plasma[] = {-4, -10, 4, -7, -4, -4, 4, -1,
                              -4, 2,   4, 5,  -4, 8,  4, 11};
static float icon_ltng[] = {-1, 9, 2, -1, -4, 1, -1, -9};
static float icon_cw[] = {-6, 7,  5, 7, 0, 10, 0,  -10,
                          -3, 10, 4, 3, 2, 10, -5, 3};
static float icon_pulse[] = {8, 0, -9, 0,  0,  9, 0, -8,
                             5, 6, -6, -5, -6, 6, 5, -5};
static float icon_apcr[] = {10, 1, 2, 5, -10, 5, -10, -4, 1, -4, 10, 1};
static float icon_apfsds[] = {-7, 2,  8,   2,  10,  1, 8,  -1,
                              -7, -1, -10, -3, -10, 4, -7, 2};
static float icon_he[] = {0,  -10, 2, -4, 8, -5, 2, 0,  8,  7,  1,  3, 0,
                          10, -2,  4, -8, 4, -3, 2, -8, -6, -3, -3, 0, -10};
static float icon_heat[] = {2,  12, 2,  5, -8,  8, -1, 2,  -9, 0, -1, -2, -8,
                            -8, 2,  -5, 2, -12, 2, -2, 10, 0,  2, 2,  2,  12};
static float icon_area[] = {9,  0,  2,  2,  0, 9,  -2, 2,  -9, 0, -2, -2,
                            0,  -9, 2,  -2, 9, 0,  6,  6,  0,  9, -6, 6,
                            -9, 0,  -6, -6, 0, -9, 6,  -6, 9,  0};
static float icon_basic[] = {0, 4, -4, 0, 0, -4, 4, 0, 0, 4};

static float icon_du_points[] = {-10, -10, -10, 10, 10, -10, 10, 10,
                                 -3,  -3,  -3,  3,  3,  -3,  3,  3};
...
// Compile prelight stuff (circle, prelight grid)
...
static void init_vertex_lists() {
    ...
    for (i = DISPLAY_LIST_BEGIN; i <= DISPLAY_LIST_TOWER_END; i++) {
        ...
        if (i == DL_TYPE(TT_DU))
            glDrawElements(lists[i].type, lists[i].size, GL_UNSIGNED_BYTE,
                           icon_du);
        else
