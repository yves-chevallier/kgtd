...
static int armor_table[][3] = {
    ...
    [TT_PLASMA] = {64, 128, 96}, [TT_LTNG] = {128, 128, 128},
    [TT_CW] = {1920, 128, 32},   [TT_PULSE] = {1280, 128, 48},
    [TT_APCR] = {128, 48, 128},  [TT_APFSDS] = {144, 48, 128},
    [TT_DU] = {192, 64, 128},    [TT_HE] = {72, 48, 128},
    [TT_HEAT] = {128, 64, 128},  [TT_AREA] = {128, 96, 96},
    [TT_BASIC] = {64, 64, 64}};

static int shield_table[] = {
    [TT_PLASMA] = 512, [TT_LTNG] = 128,  [TT_CW] = 1280,  [TT_PULSE] = 1920,
    [TT_APCR] = 64,    [TT_APFSDS] = 48, [TT_DU] = 40,    [TT_HE] = 96,
    [TT_HEAT] = 72,    [TT_AREA] = 128,  [TT_BASIC] = 48,
};

/* @brief splash range table */
static float splash_range[] = {
    [TT_PLASMA] = 0.0, [TT_LTNG] = 0.0,     [TT_CW] = 0.0,    [TT_PULSE] = 0.0,
    [TT_APCR] = 0.0,   [TT_APFSDS] = 0.0,   [TT_DU] = 0.0,    [TT_HE] = 1600.0,
    [TT_HEAT] = 900.0, [TT_AREA] = 10000.0, [TT_BASIC] = 0.0,
};
...
void damage_calc(bullet_t *bullet, int dt) {
    ...
    if (type == TT_LTNG) noob->stun_time = 800 /* ms */;

    damage =
        type <= TT_PULSE || type == TT_AREA ? damage * dt / 16384 : damage / 16;
    if (type != TT_AREA) {
    ...
    if (splash == 0.0) return;

    Q_FOREACH (cur, &noob_list, list) {
        if (noob == cur) continue;
...
/* Calculates if a target is worth firing at */
