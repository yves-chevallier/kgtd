...
#define in_main_area(x, y)                                     \
    (((x) > SEL_X + SEL_BUFFER || (y) < SEL_Y - SEL_BUFFER) && \
     (y) < LEVEL_BAR - BOT_BAR_BUFFER)
...
static void draw_main(int x, int y, state_t *state) {
    int ax = (x / GRID_SIZE) * GRID_SIZE, ay = (y / GRID_SIZE) * GRID_SIZE;
    float range = 0.0;
    grid_type g;

    if (ax > GRID_X * GRID_SIZE || ay > GRID_Y * GRID_SIZE) return;
    if (state->selected != TT_NONE) draw_prelight_grid(x, y, ax, ay);
...
static void bar_draw(int x, int y, state_t *state) {
    ...
    if (in_main_area(x, y)) {
        if (state->selected != TT_NONE)
            ...
            strcpy(buf,
                   "Click to increase power supplied, "
        ...
        if (x % BTN_SIZE > BTN_OFFSET && sy % BTN_SIZE > BTN_OFFSET) {
            ...
            if (sel->unlocked)
                ...
                snprintf(buf, sizeof(buf),
                         "Click to unlock %s tower (cost: %d)",
...
static void wave_draw_one(wave_t *wave, int dx) {
    ...
    if (wave->hp > NOOB_DEFAULT_HP * 50) glColor3f(1.0, 0.2, 0.2);
    ...
    if (wave->armor_type & ARMOR_REFLECTIVE) glCallList(DISPLAY_LIST_REFLECT);
    glPopMatrix();
}
...
static void levels_draw(state_t *state) {
    ...
    while (wave != NULL) {
        ...
        wave = wave->next;
        dx -= LEVEL_BAR_WIDTH;
        if (dx < SEL_X - LEVEL_BAR_WIDTH) break;
    }
...
void controls_init(void) {
    ...
    for (i = 0; i < 4; i++)
        for (j = 0; j < 4; j++) sel_arr[i][j].unlocked = 0;
...
void controls_draw(state_t *state) {
    ...
    if (in_main_area(x, y)) draw_main(x, y, state);
    levels_draw(state);
    scores_draw(state);
    bar_draw(x, y, state);
    sel_draw(state);
...
void controls_update(int dt, state_t *state) {
    ...
    if (state->selected != TT_NONE) return;
    if (tower->g != GRID_TYPE_TOWER) return;
