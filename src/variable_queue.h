...
#include <stddef.h>
#define Q_HEAD(Q_ELEM_TYPE)       \
    struct {                      \
        struct Q_ELEM_TYPE *head; \
        struct Q_ELEM_TYPE *tail; \
    }
#define Q_STATIC_INITIALIZER \
    { NULL, NULL }
...
#define Q_FOREACH(CUR_ELEM, Q_HEAD, LINK_NAME) \
    for (CUR_ELEM = (Q_HEAD)->head; CUR_ELEM;  \
         CUR_ELEM = (CUR_ELEM)->LINK_NAME.next)

#define Q_FOREACH_NXT(CUR_ELEM, NEXT_ELEM, Q_HEAD, LINK_NAME)       \
    for (CUR_ELEM = (Q_HEAD)->head,                                 \
         NEXT_ELEM = CUR_ELEM ? (CUR_ELEM)->LINK_NAME.next : NULL;  \
         CUR_ELEM;                                                  \
         CUR_ELEM = NEXT_ELEM,                                      \
         NEXT_ELEM = CUR_ELEM ? (CUR_ELEM)->LINK_NAME.next : NULL)
