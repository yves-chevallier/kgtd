...
/* Creates a new bullet of the given type */
...
/* Updates a projectile-type bullet */
...
/**
 * Updates a continuous-beam bullet
 *
...
void bullet_upd_cw(bullet_t *bullet, float dt, int idt) {
    ...
    if (bullet->dest->is_dead ||
        distance2(&bullet->pos, &bullet->dest->pos) > bullet->range)
        bullet->age += idt;
    else
...
void bullet_draw_all() {
    bullet_t *cur;
    Q_FOREACH (cur, &bullet_list, list) cur->draw(cur);
}
