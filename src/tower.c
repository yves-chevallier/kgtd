...
void tower_init() { Q_INIT_HEAD(&tower_list); }

tower_t *tower_new(int x, int y, unsigned int power, ttype_t type) {
    if (grid[y][x].g != GRID_TYPE_NONE) return NULL;
...
void tower_upd_normal(tower_t *tower, int dt) {
    ...
    /* de-target */
    if (target != NULL &&
        (damage_not_worthwhile(target, tower->type) ||
...
void tower_update_all(int dt) {
    ...
    Q_FOREACH (cur, &tower_list, list) cur->update(cur, dt);
}

void tower_draw_all() {
    tower_t *cur;

    Q_FOREACH (cur, &tower_list, list) draw_each(cur);
}
