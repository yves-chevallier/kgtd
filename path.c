...
enum { DIR_UP = 0, DIR_LEFT = 1, DIR_DOWN = 2, DIR_RIGHT = 3, DIR_NONE = 4 };
...
/* Finds the relative direction from the path to the given x/y */
static int rel_dir(int x, int y, path_t *rel) {
    if (rel == &(grid[y + 1][x].p)) return DIR_UP;
    if (rel == &(grid[y - 1][x].p)) return DIR_DOWN;
    if (rel == &(grid[y][x + 1].p)) return DIR_RIGHT;
    if (rel == &(grid[y][x - 1].p)) return DIR_LEFT;

    return DIR_NONE;
}

/* Draws one path square */
...
/**
 * Loads the path onto the board and generates the relevant display list
 * see level_data.c for path format description
 */
void path_load(state_t *state, map_t *map) {
    int cur_x = map->x_start, cur_y = map->y_start;
    int new_x, new_y;
...
void path_draw_all(state_t *state) { glCallList(DISPLAY_LIST_PATH); }
